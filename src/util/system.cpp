...
const char * const BITCOIN_CONF_FILENAME = "projectxcl.conf";
...
#else
    const char* pszModule = "projectxcl";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Projectxcl";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Projectxcl";
#else
    // Unix-like
    return pathRet / ".projectxcl";
#endif
#endif
}
