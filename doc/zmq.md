...
The Projectxcl Core daemon can be configured to act as a trusted "border
router", implementing the projectxcl wire protocol and relay, making
...
The ZeroMQ feature in Projectxcl Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building projectxcld:
...
For instance:
    ...
    $ projectxcld -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/projectxcld.tx.raw \
...
These options can also be provided in projectxcl.conf.
...
From the perspective of projectxcld, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into projectxcld directly. Furthermore, no information is
...
using. Projectxcld appends an up-counting sequence number to each
